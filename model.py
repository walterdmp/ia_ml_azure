...
def calcular_preco(row):
    ...
    adicional_motor = row['motor'] * 20000 
    
    fator_marca = 1 + ((row['marca'] - 1) * 0.6)
    
    idade = 2025 - row['ano']
    desconto_idade = idade * 4000  
    desconto_km = row['km'] * 0.15 
    
    valor = (preco_base + adicional_motor) * fator_marca - desconto_idade - desconto_km
    
    return max(valor, 18000)
...
print(f"Novo Modelo 'Realidade 2025' Treinado! Precisão (R²): {score:.2f}")

pickle.dump(modelo, open('model.pkl', 'wb'))
print("Arquivo 'model.pkl' atualizado com sucesso!")